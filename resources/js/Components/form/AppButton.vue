<template>
    <BasicTransition>
        <component
            ...
            :class="[buttonColor, textAlignment, buttonSize, 'flex items-center gap-2 rounded-2xl font-medium hover:shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2 transition-all']"
            @click="clicked = true"
            :preserveState
        >
            <Icon
                ...
                :class="[$props.size === 'sm' && 'size-4 mt-px', '-ml-1 h-5 w-5 animate-spin', iconColor]"
                aria-hidden="true"
            />
            <Icon v-else-if="icon" :icon :class="[$props.size === 'xs' ? 'size-1' : $props.size === 'sm' ? 'size-4 mt-px' : 'size-5', '-ml-1 size-5', iconColor]" aria-hidden="true" />
            <slot />
        </component>
    </BasicTransition>
</template>
...
const componentProps = computed(() => {
    const base = { type: $props.type, disabled: loading.value || $props.disabled || $props.forceLoading }
    if ($props.externalLinkOnly) return { ...base, href: $props.href, target: '_blank' }
...
const textAlignment = computed(() => ($props.alignment === 'l' ? 'justify-left' : 'justify-center'))

const buttonSize = computed(() => ($props.size === 'xs' ? 'text-xs font px-2 py-1' : $props.size === 'sm' ? 'text-xs font px-3 py-2' : 'text-sm px-4 py-2'))
