<template>
    <form @submit.prevent="submitForm()" class="flex flex-col gap-4">
        ...
        <div v-if="$cameraStore.taken_photos.length > 0" class="flex gap-2 overflow-x-auto">
            <button @click="newHistory('camera')" type="button" class=" bg-white rounded-xl text-center flex flex-col items-center p-8 text-neutral-600 border-2 border-dashed border-neutral-400">
            ...
            <ImagePreview
                ...
                :key="photo.id"
                :image_preview="[photo]"
                @click="photos = $cameraStore.taken_photos.map(item => { return {file_location: item.preview, id: item.id}})"
            />
        </div>
        <button v-else @click="newHistory('camera')" type="button" class="w-full bg-white py-12 flex flex-col items-center gap-2 text-neutral-400 border-2 border-dashed rounded-3xl" >
